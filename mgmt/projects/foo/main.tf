...
#
resource "aws_dynamodb_table" "foo_test" {
...
resource "aws_s3_bucket" "foo_test" {
...
#
resource "aws_iam_group" "foo_admin" {
  name = "FooAdmin"
...
resource "aws_iam_group_policy_attachment" "administrator_access_for_foo_admin" {
  group      = aws_iam_group.foo_admin.name
